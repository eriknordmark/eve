...
// Timeouts for this test. Eventually returns as soon as its condition holds,
// so a generous budget costs nothing when the system behaves and only delays a
// genuine failure; each is set from an observed duration with headroom, never
// tuned to just-barely-pass.
const (
	// clusterFormationTimeout bounds a three-node cluster forming, which is
	...
	// purgeEndStateTimeout bounds the assertion about the new generation.
	// Observed to settle within about a minute of the app coming back.
	purgeEndStateTimeout = 5 * time.Minute

	// assertPollInterval is how often the end-state assertion re-checks.
	assertPollInterval = 5 * time.Second

	// vmirsListTimeout bounds one kubectl invocation run over SSH.
	vmirsListTimeout = 20 * time.Second
)
...
const (
	...
	// the setting accepts.
	dnidOutageThreshold = 2 * time.Hour
)
...
func assertExactlyOneVMIRSAtGeneration(
	...
	names, err := dev.ListAppVMIRS(appUUID, vmirsListTimeout)
...
//     Affinity=PREFERRED.
//   - dnidOutageThresholdKey pinned past the test's own duration, so no peer
...
func TestVMAppPurgeDuringFailover(test *testing.T) {
	...
	cfgProps.SetGlobalValueInt(dnidOutageThresholdKey,
		uint32(dnidOutageThreshold.Seconds()))
	...
	for _, name := range devName {
		item := evetest.GetEdgeDevice(name).GetDeviceInfo().GetConfigItemStatus().
			GetConfigItems()[string(dnidOutageThresholdKey)]
		log.Infof("Device %q reports %s=%q (error: %q)", name,
			dnidOutageThresholdKey, item.GetValue(), item.GetError())
	}
