...
func TestVMAppPurgeAfterPowerCycle(test *testing.T) {
	...
	evetest.DefineTestParameters(
		...
		evetest.TestParameterDefinition{
			...
			Description: evetest.TestParameterDescription{
				...
				// cmd/list-tests reads this field out of the AST and resolves only a
				// plain string literal, so a concatenation of the consts above would
				// leave "make list-tests" printing no allowed values at all.
				AllowedValues: "power-cycle|controller-reboot",
			},
		},
	)
	...
	case powerCutControllerReboot:
		// Subscribe before the node goes down, so the first post-boot info
		// message cannot be missed while the wait below looks for it. Nothing
		// reads this channel on the power-cycle path, so it is scoped to the
		// only branch that consumes it.
	...
	}, postRebootEndStateTimeout, assertPollInterval).Should(Succeed())
