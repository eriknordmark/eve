...
// stops it. The point is a guest which ignores the poweroff request, not one
// which races cloud-init for it.
...
//
// Nothing obliges a guest to service that request: it may be wedged, or shutting
// down something of its own, or simply not running anything which listens. When
// it does not, only terminating the domain ends the stop, and until then the
// application stays reported as halting and holds its memory and any assigned
// PCI devices. The guest here ignores the request by construction -- acpid is
// what services the power button on Alpine, and cloud-init stops it and removes
// it from the runlevel.
//
// The mode is HVM, which already gets the short first wait, so the whole
// difference is what the escalation after it does.
//
// Network model
// -------------
