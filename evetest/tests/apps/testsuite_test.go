...
// (TestVMAppPurgeBaseline) and a purge issued while the device is powered off
// (TestVMAppPurgeAfterPowerCycle - a deterministic reproduction of a
// reboot-during-purge bug).
//
...
//
// TestVMAppPurgeBaseline is pinned to Kubevirt (eve-k). The power-cycle test
// runs twice, once per hypervisor: Kubevirt is where the duplicate-generation
// defect lives, and kvm is the control that localises it there - see that
// test's own comment for what each variant can and cannot assert.
