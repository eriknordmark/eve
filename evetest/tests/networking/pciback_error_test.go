...
// Standalone (a different network model than the TwoMgmtPorts suite).
...
// TestPcibackErrorSuite groups the assignable-adapter error-reporting scenarios
// (all on the TwoMgmtPorts model). TestReportWarningsOnly runs separately since
// it needs a four-port model.
