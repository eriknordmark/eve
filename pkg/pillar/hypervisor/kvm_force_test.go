...
// looked for under that directory, so nothing here can reach a domain running
// on the test host.
func startSleeper(t *testing.T) (stateDir string, domainName string, pid int) {
	...
	pid = cmd.Process.Pid
	// The stand-in is a child of the test, so it has to be reaped as soon as it
	// dies: an unreaped child stays a zombie, and a zombie still answers
	// signal 0, which is how liveness is probed. qemu is not a child of pillar,
...
// A domain whose QMP monitor cannot be reached still has to be terminated: this
// is the path taken when a guest never services the poweroff request, which is
// what a forced shutdown exists for. Without the kill backstop the process
// survives and the caller keeps seeing the domain as present.
func TestTerminateQemuKillsWhenMonitorUnreachable(t *testing.T) {
	stateDir, domainName, pid := startSleeper(t)

	// No qmp socket exists under the temporary state directory, so the QMP
	// quit is skipped exactly as it is for a wedged or absent monitor.
...
// not wedge the caller. There is nothing left to terminate in that case, so it
// is not reported as a failure either.
...
// A pid file that is there but unusable leaves no handle to signal, so no kill
// is attempted at all. The caller has to hear about that, otherwise a forced
// stop is reported as having succeeded over a domain that is still running.
