...
type RequestOptions struct {
	...
	// CustomHeader, if set, replaces the default HTTP headers normally
	// generated by the client for this request. This allows callers to
	// fully control the outgoing request headers.
...
func (c *Client) SendOnIntf(ctx context.Context, destURL string, intf string,
	...
	for i, addr := range portStatus.AddrInfoList {
		...
		req, reqlen, err := c.prepareHTTPRequest(
			reqURL, b, isEdgenode, isGet, useOnboard, opts.CustomHeader)
		if err != nil {
			errorLog(err.Error())
			attempt.Err = err
...
func (c *Client) prepareHTTPRequest(reqURL string, b *bytes.Buffer, isEdgenode, isGet,
	useOnboard bool, customHeader http.Header) (req *http.Request, reqlen int64, err error) {
	var b2 *bytes.Buffer
	if isEdgenode && !isGet {
