...
// partition. Returns false if it is absent or unparsable.
...
// declineConversion abandons the boot-disk conversion with reason, leaving the
// device on its current flavor: it clears the converting state, records the
// error on BaseOsStatus and publishes it. It always returns false, so a caller
// can `return declineConversion(...)` to tell maybeConvert's caller that the
// install must not proceed and that status has already changed.
func declineConversion(ctx *baseOsMgrContext, status *types.BaseOsStatus, reason string) bool {
	log.Error(reason)
...
func maybeConvert(ctx *baseOsMgrContext, status *types.BaseOsStatus) bool {
	...
	if err != nil {
		return declineConversion(ctx, status,
			fmt.Sprintf("conversion: cannot determine boot disk: %s", err))
	}
	...
	if mountErr != nil {
		return declineConversion(ctx, status,
			fmt.Sprintf("conversion: cannot access CONFIG partition: %s", mountErr))
	}
	if clearedStale {
	...
	if declineReason != "" {
		return declineConversion(ctx, status, declineReason)
	}
	...
	case diskconvert.OutcomeInsufficient:
		return declineConversion(ctx, status,
			fmt.Sprintf("conversion not possible: %s", res.Reason))

	default:
		return declineConversion(ctx, status,
			fmt.Sprintf("conversion failed: %v", runErr))
	}
}
