...
func unpublishBlobStatus(ctx *volumemgrContext, blobs ...*types.BlobStatus) {
	...
	for _, blob := range blobs {
		...
		if blobDeferredProtected(ctx, blob.Sha256) {
			log.Noticef("BLOBGC unpublishBlobStatus(%s): kept (refcount=%d), protected by deferred-delete record", key, blob.RefCount)
			publishBlobStatus(ctx, blob)
			continue
		}
	...
	for _, image := range casImages {
		if _, ok := referenceMap[image]; !ok {
			...
			if imageDeferredProtected(ctx, image) {
				log.Noticef("BLOBGC gcImagesFromCAS: keeping image %s, protected by deferred-delete record", image)
				continue
			}
