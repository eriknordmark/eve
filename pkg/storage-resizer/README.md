...
### Who decides how much shrink is safe — the `--max-full` policy

Shrinking is **not** gated on a flat free-space margin. Freeing `--need` (22 GB)
from a filesystem that is already fairly full would land it near 100% full,
where ext4 metadata overhead and fragmentation make `resize2fs` unreliable and
leave no operating headroom. Instead, `check` bounds the **resulting** fullness:

> a shrink is allowed only if, after freeing `--need`, the filesystem would be
> at most **`--max-full`%** full (default **90**).

So `used ≤ max-full% × (currentSize − need)`. Example: a 100 GB persist at 70 GB
used, freeing 22 GB → 78 GB result → 89.7% full → allowed at 90; the same fs at
75 GB used → 96% → rejected. As the filesystem fills, the amount we are willing
to free shrinks accordingly, and a too-full device is correctly reported
`insufficient` (controller-visible error; the device stays on its current
