...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  storage-resizer backup  [--persist /persist] [--backup-dir /config/backup-persist] [--flag-file /config/shrink-persist] --target 78G")
	fmt.Fprintln(os.Stderr, "  storage-resizer shrink  --disk <path> (--shrink-to 78G | --flag-file /config/shrink-persist) [--dry-run]   (storage-init/offline)")
	fmt.Fprintln(os.Stderr, "  storage-resizer grow    --disk <path> [--esp 2G] [--imga 10G] [--imgb 10G] [--dry-run]                     (baseosmgr/online)")
	fmt.Fprintln(os.Stderr, "  storage-resizer restore [--persist /persist] [--backup-dir /config/backup-persist] [--flag-file /config/shrink-persist] [--cleanup]")
	fmt.Fprintln(os.Stderr, "  storage-resizer cleanup [--backup-dir /config/backup-persist] [--flag-file /config/shrink-persist]")
}
...
func printHuman(rep checkReport) {
	...
	if rep.Shrink != nil {
		fmt.Printf("space-to-shrink-ext: %v (used %s of %s; after freeing %s -> %s, result %d%% full, max %d%%)\n",
