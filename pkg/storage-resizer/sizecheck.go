...
// maxFullPercent full.
//
// A flat free-space check is not enough: shrinking a filesystem that is, say,
// 75% of a 100 GB partition down by 22 GB lands it at ~96% full, where ext4
// metadata overhead and fragmentation make resize2fs unreliable and leave the
// device with no operating headroom. This bounds the *resulting* fullness
// instead, so the amount we are willing to free shrinks as the filesystem fills.
func SpaceToShrinkExt(path string, partOffset, needBytes int64, maxFullPercent int) (ShrinkResult, error) {
	...
	// OK iff the shrunk filesystem would be at most maxFullPercent full.
	r.OK = used*100 <= int64(maxFullPercent)*target
	return r, nil
}
