...
EVE supports two complementary sources of Go basic-block code coverage that
can be merged into a single combined report:
...
# Extra dirs alongside Eden E2E (all three sources merged)
make coverage-merge EXTRA_COVERAGE_DIR=/path/to/hw-test-cov
```
...
header followed by all coverage lines from both profiles.  Lines covering
