...
ifeq ($(HV),k)
        ...
        ifeq (, $(findstring nvidia,$(PLATFORM)))
            # STRESS BRANCH ONLY: 290 upstream. The -tags chaos storage-resizer is
            # ~1MB larger than the stock binary, tipping the kvm rootfs just over
            # 290MB. Bump to 295 -- still under the 300MB pre-10.2.0 hard limit noted
            # above (older devices have a 300MB rootfs partition). Must never land
            # upstream.
