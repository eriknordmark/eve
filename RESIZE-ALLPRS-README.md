...
- `pkg/storage-resizer` is content-hash pinned by both `pkg/pillar/Dockerfile`
  and `pkg/storage-init/Dockerfile`. This branch inherits #6063's
  `7676796214b5d9f2ed6321b644c06a8e9747e22b`, re-verified with
  `build-tools/bin/linuxkit pkg show-tag pkg/storage-resizer`. The stress twin's
  value differs — never copy a pin across branches.
