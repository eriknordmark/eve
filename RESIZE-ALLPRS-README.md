...
with the same PR contribution plus the fault-injection gates (fork#7); the two
are siblings off the same base, not stacked.
...
| change | why |
|--------|-----|
| Makefile: rootfs cap 295MB | the integration content exceeds the upstream 290 cap; still under the 300MB pre-10.2.0 hard limit |
