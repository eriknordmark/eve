# newgo-allprs2-stress-oldkernel — multi-PR integration branch

Integration vehicle: it combines still-open PRs so one build and test run sees
their combined diff. **Never PR'd upstream.**

Base: `upstream/master` @ `664756969`. Head `ce511e8e5`, 81 commits on top.
...
| fork#7 | `resize-watchdog-stress` | `e303cf4de` | DO NOT MERGE. The --no-pet escalating watchdog in storage-init, watchdog-during-GPT-write chaos instrumentation in storage-resizer, and the pins that ship the -tags chaos binary |
...
| Pin amd64 kernel back to 6.12.49 | this line tests against the older kernel |
